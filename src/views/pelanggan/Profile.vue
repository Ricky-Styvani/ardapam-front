...
export default {
  ...
  methods: {
    ...
    getData() {
      axios
        .get(`${this.$store.state.host}/api/showuser/${this.id}`, { headers: { Authorization: `Bearer ${window.localStorage.getItem("token")}` } })
        .then((res) => {
    ...
    update() {
      ...
      axios
        .patch(`${this.$store.state.host}/api/updateuser/${this.id}`, data, { headers: { Authorization: `Bearer ${window.localStorage.getItem("token")}` } })
        .then(() => {
          this.$swal.fire("Success!", "profile updated!.", "success");
        })
        .catch((err) => {
          console.log({ err });
        });
